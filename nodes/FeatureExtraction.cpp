...
int main (int argc, char **argv)
{
    ...
    //string lib_path = "/home/alexis/catkin/src/features_face/lib/local";
    vector<string> arguments;
    arguments.push_back(lib_path + "/LandmarkDetector");
    ...
    // HACK : it should be passed as an argument or throw error
	int device = 1;
	...
	{
        path loc = path(lib_path) / "LandmarkDetector/model/tris_68_full.txt";
		tri_loc = loc.string();
        cout<<tri_loc<<endl;
	...
	if (dynamic)
	{
		au_loc_local = "FaceAnalyser/AU_predictors/AU_all_best.txt";
	}
	else
	{
		au_loc_local = "FaceAnalyser/AU_predictors/AU_all_static.txt";
	}
	...
	{
		path loc = path(lib_path) / au_loc_local;
	...
	const FaceAnalysis::FaceAnalyser& face_analyser, tf::Transform tr, tf::TransformBroadcaster br)
{
  ...
  // Output the detected 3D facial landmarks
    ...
    */
    ...
    x = pose_estimate[0]*1.2/1000.; // HEAD TX
    y = pose_estimate[1]*1.2/1000.; // HEAD TY
    z = pose_estimate[2]*1.2/1000.; // HEAD TZ
    ...
    //add a tf frame for detected face
